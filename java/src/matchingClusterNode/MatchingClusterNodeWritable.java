 ...
 * "query matching clusters", i.e. clusters that contain a node with all terms
 * of a query. To allow writing final results, the data is expanded with
 * docuemnetID and sentence number.
 *
 * @author jeroen
 */
public class MatchingClusterNodeWritable extends Writable<MatchingClusterNodeFile> {
    ...
    /**
     * 
     * @return a shallow clone of the object in the sense that the nearest neighbor arrays are shared
     * therefore assignments to the clone are also reflected on the original. 
     */
    @Override
    public MatchingClusterNodeWritable clone() {
        MatchingClusterNodeWritable clone = new MatchingClusterNodeWritable();
        clone.clusterID = clusterID;
        clone.creationTime = creationTime;
        clone.documentID = documentID;
        clone.domain = domain;
        clone.nnid = nnid;
        clone.nnscore = nnscore;
        clone.sentenceNumber = sentenceNumber;
        clone.content = content;
        clone.sentenceID = sentenceID;
        return clone;
    }
